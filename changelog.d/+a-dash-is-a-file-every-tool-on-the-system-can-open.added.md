...
operating system existed.

`intake.slurp` now treats `-` as standard input, streamed to the same 64 MB
ceiling a named file gets, through the same single seam every command already
reads with - so `parse`, `amend`, and `mint` all inherited the spelling at
once rather than each growing its own.
