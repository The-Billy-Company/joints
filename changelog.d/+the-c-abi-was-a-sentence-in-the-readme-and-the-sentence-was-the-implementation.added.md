"The C ABI is `libotl`, symbols prefixed `otl_`" has been a sentence in the
...
The door exists now: **32 `otl_*` exports** behind `include/otl.h`, built as
both `libotl.dylib`/`.so` (which owns the header install, for the dlopen and
cffi crowd) and a `libotl.a` that **links standing alone** - the archive is
...
The surface is three handles in a strict lifetime order - a `otl_bank` (a
folio, a codex, or a `grammar.json` pressed on open), an `otl_parser` lent out
of it for one language, `otl_tree`s handed back - plus node accessors that
...
`otl_last_error()` holds the sentence the CLI would have printed, per thread.
A parse returns a tree on **every** success, accepted or not: `otl_tree_stop`
...
is refused (`OTL_LANGUAGE`) for a language the bank does not hold, parses,
walks the tree, renders the same s-expression the CLI prints, gets
`OTL_TRUNCATED` with a tree attached for a cut-off file, and gets `OTL_IO`
