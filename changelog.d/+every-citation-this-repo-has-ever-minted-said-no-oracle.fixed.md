...
and the oracle is a third of that world. It printed `**no oracle** - joints's
...
```text
joints `1885792a7` · tree `4f018b60f` (pin) · oracle `d85e736fa` (30 attributed)
```
...
what a bare `--cite` beside `JOINTS_BIN=<a pin>` now says, correctly, because
...
`collate` each already spell `JOINTS_WORK or ROOT/.local/standing` and a
