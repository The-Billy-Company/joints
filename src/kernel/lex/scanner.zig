...
/// Transcribe a customary from its JSON into the bytes a folio carries.
///
/// The one thing about the customary this file does publish, and it is a
/// function rather than a type, so `idiom`'s ownership census still reads
/// `Engine` exactly once. It is here because the seal on this directory means a
/// caller that is not the facade needs a named way in, and `press/docket` is
/// that caller: its folio round trip has to carry a section a real
/// `Engine.bind` accepts, and only a real transcription makes one.
pub const transcribe = customary.press;

/// The book this binary ships for `name`, or null for a grammar with none.
///
/// The shelf is generated by `build.zig` off `customary/*.json`, so this is a
/// walk over an array of eight and not a lookup worth indexing - it runs once
/// per scanner built, against a table whose length is the number of grammars
/// whose scanner has been transcribed. When that number is thirty it is still
/// once per scanner.
fn shelved(name: []const u8) ?[]const u8 {
    for (@import("customary_shelf").shelf) |b| {
        if (std.mem.eql(u8, b.name, name)) return b.json;
    }
    return null;
}
...
pub const CompileError = error{
    ...
    BadPattern,
    // `PressError` as well as `Error`: a grammar that arrived without a book gets
    // the one this binary ships, and pressing it is work this function now does
    // rather than work `intake` did before calling it. A shelved book is ours and
    // gated, so these are bugs rather than input faults - which is the argument
    // for surfacing them here rather than swallowing them and falling back to a
    // hand, since a book that stopped pressing would then look like a book that
    // was never written.
} || customary.Error || customary.PressError || std.mem.Allocator.Error;
...
pub const Scanner = struct {
    ...
    /// Externals a hand answers, and externals this grammar's customary claims.
    /// Ascending, because the terminal walk that fills them is. Neither is a
    /// blindness - both are answered - and they are here because being answered
    /// is not the same as being *seated*: no pattern in the slate covers these
    /// bytes, so a wall on one of them is neither `nothing_lexes` nor a table's
    /// doing. It is the scanner declining at that offset, and the two lists say
    /// which scanner, which is the difference between an ask of our Zig and an
    /// ask of one rule in `customary/<grammar>.json`. `press/inquest.zig` reads
    /// them for exactly that; see `Because.awaited_hand` and its sibling.
    handed: []const press.Symbol,
    transcribed: []const press.Symbol,
    ...
    pub fn compile(gpa: std.mem.Allocator, gr: *const press.Grammar) CompileError!?Scanner {
        ...
        // The two populations that are answered but not seated. Split from
        // `blind` for the reason `declined` was: they have different owners.
        var handed: std.ArrayList(press.Symbol) = .empty;
        var transcribed: std.ArrayList(press.Symbol) = .empty;
        ...
        // A grammar that arrived without a book still has one if we ship it, and
        // the shelf is where the binary keeps them. This is the floor of the four
        // ways to a book, under the three `intake.customary` documents, and it is
        // the one that makes a hand retirable: a fallback reachable only when no
        // path was typed is a fallback every entry point still needs, so the C
        // ABI and every embedder found no book at all until this branch existed.
        //
        // Pressed here rather than held pressed, because `bind` copies the
        // section into its own aligned allocation - so the wire bytes are scrap
        // the moment it returns, and holding them would be holding a second copy
        // of what the engine already owns.
        const pressed: ?[]align(customary.alignment) u8 = if (gr.customary.len > 0)
            null
        else if (shelved(gr.name)) |json|
            try customary.press(gpa, json, null)
        else
            null;
        defer if (pressed) |p| gpa.free(p);
        const wire: []const u8 = pressed orelse gr.customary;

        var bound: ?customary.Engine = if (wire.len == 0)
            null
        else
            try customary.Engine.bind(gpa, wire, names);
        ...
        for (0..gr.terminal_count) |i| {
            ...
            // Who answers this external, and the three answers are not one
            // fact: a customary is data in this grammar's own folio, a hand is
            // our Zig, and nothing is nobody's. Which of the three it is
            // decides whose work order a wall on it becomes, so the walk
            // records it rather than collapsing all three into "not blind".
            var answered = false;
            if (pattern == .external) {
                const name = gr.nameOf(sym);
                if (bound) |*b| if (b.claims(sym)) {
                    answered = true;
                    try transcribed.append(gpa, sym);
                };
                for (casts.items) |*c| {
                    if (!outside.claimed(c.troupe, name)) continue;
                    // Where both answer, `outside.step` prefers the
                    // transcription, so the customary owns the report too.
                    if (!answered) try handed.append(gpa, sym);
                    answered = true;
                }
                if (!answered) if (outside.provisionFor(gr, name)) |p| {
            ...
            switch (pattern) {
                ...
                .external => if (!answered) try blind.append(gpa, sym),
                .literal => |lit| {
                    literal.set(i);
        ...
        errdefer handed.deinit(gpa);
        errdefer transcribed.deinit(gpa);
        ...
        return .{
            ...
            .handed = try handed.toOwnedSlice(gpa),
            .transcribed = try transcribed.toOwnedSlice(gpa),
    ...
    test "a save stays small enough to hang one on every spine ring" {
        ...
        // It used to sit close under the page because html's element ancestry was
        // in here *twice* while both scanners for it existed - once as
        // `lineage.Tags`, once as the customary's marks. The transcription is
        // trusted now and the hand is gone, so the duplicate is too; what is left
        // is the customary's copy, which every grammar with a book shares.
    ...
    /// Where this scan is standing, in one word - `Save` priced for a record
    /// kept per token rather than per ring.
    ///
    /// Zero for a scanner with no memory to stand in, which is most of them:
    /// the offset alone says what such a scanner reads next, so every stance
    /// is the same stance and a comparison of two of them has nothing to say.
    /// It also keeps the hash off the hot path for json and its kin.
    pub fn stance(s: *const Scanner) u64 {
        return if (s.outward()) s.carry.digest() else 0;
    }

    /// The one carried offset, beside `stance` rather than in it. Zero on a
    /// scanner with no memory, and on one whose hand has never answered with
    /// extent - which is the same reading either way: no answer to be behind.
    pub fn since(s: *const Scanner) u32 {
        return if (s.outward()) s.carry.since() else 0;
    }

    /// Move that offset to where a scan of skipped bytes would have left it.
    pub fn rebase(s: *Scanner, at: u32) void {
        if (s.outward()) s.carry.handed(at);
    }
