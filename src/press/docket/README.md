...
tests do the same thing: not one of them checks the press by reading the press.
...
| `quotient_test.zig` | The state relation against the table it claims to describe, and then against the folio section it was written into. It arrived loose in `press/` and re-earned the exact debt the four variances above had just paid off - reading `folio` to check that a section round-trips is the whole point of it, and that is an integration test's reach, not a unit test's. |
