...
//!
//! Four moving parts. `leaf` is the on-disk vocabulary both sides speak,
//! `impose` writes, `collate` reads and validates the layout, `audit` validates
//! what is inside it. The seam is here so the lane building it never has to
//! touch `src/root.zig`.
//!
//! ```
...
//! const f = try folio.open(bytes);                         // or map(dir, …)
//! std.debug.print("{s}: {d} states\n", .{ f.title(), f.head.state_count });
//! ```
